...
exports.CDP_FACILITATOR_URL = void 0;
exports.createCdpFacilitatorClient = createCdpFacilitatorClient;
/*
...
exports.CDP_FACILITATOR_URL = "https://api.cdp.coinbase.com/platform/v2/x402";
const SDK_METADATA = {
 ...
 */
const resolveFacilitatorEndpoints = (baseUrl) => {
    let parsed;
    try {
        parsed = new URL(baseUrl);
    }
    catch {
        throw new Error(`Invalid facilitator baseUrl "${baseUrl}": must be a valid URL (e.g. "https://api.cdp.coinbase.com/platform/v2/x402").`);
    }
...
const getEndpointAuthHeaders = async (credentials, host, path, method = "POST") => {
    const jwt = await (0, index_js_1.generateJwt)({
...
const createCdpAuthHeaders = (credentials, host, paths) => {
    return async () => {
 ...
 * ```
 */
function createCdpFacilitatorClient(args) {
    ...
    const credentials = { apiKeyId: apiKeyId, apiKeySecret: apiKeySecret };
    const { url, host, paths } = resolveFacilitatorEndpoints(args?.baseUrl ?? exports.CDP_FACILITATOR_URL);
