...
export class SpendTracker {
    ...
    constructor(options = {}) {
        ...
        if (!Number.isInteger(this.maxLedgerEntries) || this.maxLedgerEntries <= 0) {
            throw new SpendControlError("configuration_invalid", `maxLedgerEntries must be a positive integer, received ${this.maxLedgerEntries}`, { input: options.maxLedgerEntries });
        }
    }
    /**
    ...
    async record(input) {
        ...
        if (currentSize >= this.maxLedgerEntries) {
            throw new SpendControlError("ledger_capacity_exceeded", `SpendTracker ledger is full (maxLedgerEntries=${this.maxLedgerEntries}); rejecting new spend record to prevent under-counting.`);
        }
    ...
    async total(query) {
        ...
        for (const entry of entries) {
            if (query.since !== undefined && entry.at < query.since)
                continue;
    ...
    async removeEntry(entry) {
        ...
        if (!this.warnedOnRemove) {
            ...
            console.warn(`[@coinbase/cdp-sdk/x402] SpendTracker: store does not implement removeEntry; ` +
